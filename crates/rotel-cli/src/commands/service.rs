//! Service management commands for running rotel as a background daemon
...
fn get_pid_file() -> Result<PathBuf> {
    Ok(get_runtime_dir()?.join("rotel.pid"))
}

/// Get the path to the log file
fn get_log_file() -> Result<PathBuf> {
    Ok(get_runtime_dir()?.join("rotel.log"))
}
...
pub async fn handle_start(storage_path: String, addr: String) -> Result<()> {
    ...
    if let Some(pid) = read_pid()? {
        if is_process_running(pid) {
            return Err(Error::ConfigError(format!(
                "Rotel is already running with PID {}",
                pid
            )));
        } else {
    ...
    println!("✓ Rotel daemon started with PID {}", pid);
    println!("  Logs: {}", log_file.display());
    println!("  Storage: {}", storage_path);
    println!("  Dashboard: http://{}", addr);
    println!("\nUse 'rotel stop' to stop the daemon");
    println!("Use 'rotel status' to check daemon status");

    Ok(())
}

/// Stop the rotel daemon
pub async fn handle_stop() -> Result<()> {
    let pid = read_pid()?.ok_or_else(|| {
        Error::ConfigError("Rotel daemon is not running (no PID file found)".to_string())
    })?;
    ...
    if !is_process_running(pid) {
        ...
        return Err(Error::ConfigError(
            "Rotel daemon is not running".to_string(),
        ));
    }

    info!("Stopping rotel daemon (PID {})...", pid);
...
/// Install rotel as a system service
...
/// Install rotel as a launchd service on macOS
#[cfg(target_os = "macos")]
async fn install_launchd_service() -> Result<()> {
    ...
    let plist_path = launch_agents_dir.join("dev.rotel.daemon.plist");
    let exe_path = std::env::current_exe()
        .map_err(|e| Error::ConfigError(format!("Failed to get executable path: {}", e)))?;

    let log_file = get_log_file()?;
    let storage_path = get_runtime_dir()?.join("rotel.db");
...
<dict>
    <key>Label</key>
    <string>dev.rotel.daemon</string>
...
/// Install rotel as a systemd service on Linux
#[cfg(target_os = "linux")]
async fn install_systemd_service() -> Result<()> {
    ...
    let unit_path = systemd_user_dir.join("rotel.service");
    let exe_path = std::env::current_exe()
        .map_err(|e| Error::ConfigError(format!("Failed to get executable path: {}", e)))?;
    ...
    let unit_content = format!(
        r#"[Unit]
Description=Rotel OpenTelemetry Collector
...
"#,
    ...
    println!("  systemctl --user enable rotel.service");
    println!("  systemctl --user start rotel.service");
    println!("\nTo check service status:");
    println!("  systemctl --user status rotel.service");
    println!("\nTo disable the service:");
    println!("  systemctl --user stop rotel.service");
    println!("  systemctl --user disable rotel.service");
