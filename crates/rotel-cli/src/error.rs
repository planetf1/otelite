    ...
    fn from(err: reqwest::Error) -> Self {
        if err.is_connect() || err.is_timeout() {
            Error::ConnectionError(format!(
                "Failed to connect to Rotel backend. Is the server running? Error: {}",
                err
            ))
...
mod tests {
    ...
    fn test_user_message() {
        ...
        assert!(msg.contains("Check if Rotel server is running"));
