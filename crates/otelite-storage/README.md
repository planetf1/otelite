...
- **Zero Configuration**: Automatically initializes storage at `~/.otelite/data` with no setup required
...
```rust
use otelite_core::telemetry::{LogRecord, Span, Metric};
...
```rust
use otelite_storage::PurgeOptions;
...
```bash
# Run all tests
cargo test --package otelite-storage

# Run specific test suite
cargo test --package otelite-storage --test zero_config_test
cargo test --package otelite-storage --test persistence_test

# Run with output
cargo test --package otelite-storage -- --nocapture
```
