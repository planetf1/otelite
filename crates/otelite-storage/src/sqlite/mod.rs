...
impl StorageBackend for SqliteBackend {
    ...
    async fn query_cost_by_project_model_tool(
        ...
        self.read_query(move |conn| {
            reader::query_cost_by_project_model_tool(conn, start_time, end_time)
                .map_err(StorageError::from)
        })
        .await
    }

    async fn query_session_depth_storage(
        &self,
        start_time: Option<i64>,
        end_time: Option<i64>,
    ) -> Result<otelite_core::api::SessionDepthStorageResponse> {
        self.read_query(move |conn| {
            reader::query_session_depth_storage(conn, start_time, end_time)
                .map_err(StorageError::from)
        })
        .await
    }
    ...
    async fn query_loc_efficiency(
        ...
        self.read_query(move |conn| {
            reader::query_loc_efficiency(conn, start_time, end_time).map_err(StorageError::from)
        })
        .await
    }
    ...
    async fn query_rare_tool_sessions(
        ...
        self.read_query(move |conn| {
            reader::query_rare_tool_sessions(conn, start_time, end_time).map_err(StorageError::from)
        })
        .await
    }
    ...
    async fn query_tool_switch_storage(
        ...
        self.read_query(move |conn| {
            reader::query_tool_switch_storage(conn, start_time, end_time)
                .map_err(StorageError::from)
        })
        .await
    }
    ...
    async fn query_session_chain_storage(
        ...
        self.read_query(move |conn| {
            reader::query_session_chain_storage(conn, start_time, end_time)
                .map_err(StorageError::from)
        })
        .await
    }
