...
- **Thinking & Effort** is now one report with per-tool sections — Claude
  ...
  names its emitter population and degrades independently
...
- `otelite retries` — lists retried LLM calls (a failed first attempt that
  ...
  GenAI Analytics page (`/api/genai/retries`), with one-click jumps from
  each row to the session's traces and the trace detail (spans, logs,
  session report) — "when did my streaming retries happen" is one command
